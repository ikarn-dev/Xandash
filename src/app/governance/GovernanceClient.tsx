...
const MemoizedProposalsTab = React.memo(ProposalsTab);
const MemoizedTreasuryTab = React.memo(TreasuryTab);
const MemoizedMembersTab = React.memo(MembersTab);
...
export function GovernanceClient() {
  ...
  const memoizedData = React.useMemo(() => {
    ...
    return {
      ...
      treasuryValueFormatted: formatUsd(data.stats.treasuryValueUsd),
    };
  }, [data]);
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="relative bg-black border border-white/10 overflow-hidden">
        ...
        <div className="p-3 sm:p-4 md:p-6">
          {activeTab === 'proposals' && <MemoizedProposalsTab data={memoizedData} />}
          {activeTab === 'treasury' && <MemoizedTreasuryTab data={memoizedData} />}
          {activeTab === 'members' && <MemoizedMembersTab data={memoizedData} />}
          {activeTab === 'info' && <MemoizedDAOInfoTab data={memoizedData} />}
        </div>
      </div>

      {/* Footer */}
      <div className="text-center text-white/30 text-[10px] sm:text-xs py-2">
        Updated: {new Date(memoizedData.fetchedAt).toLocaleString()} • Solana blockchain via Helius
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-black border border-white/10 p-4 sm:p-6">
        <div className="h-6 sm:h-8 bg-white/10 rounded w-1/3 mb-2 animate-pulse"></div>
        <div className="h-3 sm:h-4 bg-white/10 rounded w-2/3 animate-pulse"></div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-black border border-white/10 p-3 sm:p-4">
            <div className="h-3 sm:h-4 bg-white/10 rounded w-1/2 mb-2 animate-pulse"></div>
            <div className="h-6 sm:h-8 bg-white/10 rounded w-3/4 animate-pulse"></div>
          </div>
        ))}
      </div>
      <div className="bg-black border border-white/10 p-4 sm:p-6">
        <div className="h-8 bg-white/10 rounded w-full mb-4 animate-pulse"></div>
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-12 sm:h-16 bg-white/10 rounded animate-pulse"></div>
          ))}
        </div>
      </div>
    </div>
