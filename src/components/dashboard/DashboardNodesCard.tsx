    ...
    <div className="relative bg-black border border-white/10 p-4 sm:p-6 h-full group hover:border-white/20 transition-all duration-300 overflow-hidden">
      ...
      {selectedForCompare.length > 0 && typeof document !== 'undefined' && createPortal(
        <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-2 bg-black/95 border border-emerald-500/30 rounded-full px-3 sm:px-4 py-2 shadow-lg shadow-emerald-500/20 backdrop-blur-xl animate-blur-reveal safe-area-bottom">
          <div className="flex items-center gap-2">
            <div className="flex -space-x-1">
              {selectedForCompare.slice(0, 4).map((_, i) => (
                <div key={i} className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-emerald-500/20 border-2 border-emerald-500 flex items-center justify-center text-[9px] sm:text-[10px] text-emerald-400 font-bold">{i + 1}</div>
              ))}
            </div>
          ...
          <button onClick={handleClearCompare} className="px-2 sm:px-3 py-1 sm:py-1.5 text-xs text-white/60 hover:text-white hover:bg-white/10 rounded-full transition-colors">Clear</button>
          <button onClick={handleCompareSelected} disabled={selectedForCompare.length < 2}
            className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all ${selectedForCompare.length >= 2 ? 'bg-emerald-500 text-white hover:bg-emerald-400' : 'bg-white/10 text-white/40 cursor-not-allowed'}`}>
            <CompareIcon className="w-3 h-3 sm:w-4 sm:h-4" />Compare
          </button>
        </div>,
        document.body
      )}
