...
export function ComparePageContent() {
  ...
  const triggerAutoCompare = async (pubkeys: string[], nodes: NodeData[], timestamp: number) => {
    ...
    try {
      const profiles: NodeProfile[] = pubkeys
        .map((pubkey, i) => {
      ...
      setNodeProfiles(prev =>
        prev.map(profile => ({
          ...profile,
          location: locationMap[profile.ip]
            ? {
                country: locationMap[profile.ip].country,
                city: locationMap[profile.ip].city,
                provider: locationMap[profile.ip].provider
              }
            : undefined,
          history: (historyByIp[profile.ip] || []).map((h: any) => ({
  ...
  const handleToggleNode = useCallback((pubkey: string) => {
    setSelectedPubkeys(prev =>
      prev.includes(pubkey) ? prev.filter(p => p !== pubkey) : prev.length >= 4 ? prev : [...prev, pubkey]
    );
  }, []);

  const handleToggleCountry = useCallback((countryCode: string) => {
    setSelectedCountryCodes(prev =>
      prev.includes(countryCode) ? prev.filter(c => c !== countryCode) : prev.length >= 4 ? prev : [...prev, countryCode]
    );
  }, []);
  ...
  const handleCompareNodes = useCallback(async () => {
    ...
    try {
      const profiles: NodeProfile[] = selectedPubkeys
        .map((pubkey, i) => {
  ...
  }, [selectedPubkeys, allNodes, serverTimestamp, network, fetchManagerAssets]);

  const handleCompareCountries = useCallback(() => {
    if (selectedCountryCodes.length < 2) return;
    setIsComparing(true);

    const profiles: CountryProfile[] = selectedCountryCodes
      .map((code, i) => {
  ...
  const handleResetCountries = useCallback(() => {
    setSelectedCountryCodes([]);
    setCountryProfiles([]);
    setShowCountryResults(false);
  }, []);
  ...
  const handleTypeChange = useCallback(
    (type: 'nodes' | 'countries') => {
      setCompareType(type);
      handleResetNodes();
      handleResetCountries();
    },
    [handleResetNodes, handleResetCountries]
  );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div
              ...
              className={`relative bg-black border p-5 sm:p-6 cursor-pointer transition-all duration-300 group ${
                compareType === 'nodes' ? 'border-emerald-500/50' : 'border-white/10 hover:border-white/20'
              }`}
            >
              <CornerAccents />
              <div className="flex items-start gap-4">
                <div
                  className={`p-3 rounded-lg ${
                    compareType === 'nodes' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-white/40'
                  }`}
                >
                  <NodesIcon />
                </div>
                ...
                {compareType === 'nodes' && <div className="w-2 h-2 rounded-full bg-emerald-500" />}
              </div>
            ...
            <div
              ...
              className={`relative bg-black border p-5 sm:p-6 cursor-pointer transition-all duration-300 group ${
                compareType === 'countries' ? 'border-purple-500/50' : 'border-white/10 hover:border-white/20'
              }`}
            >
              <CornerAccents />
              <div className="flex items-start gap-4">
                <div
                  className={`p-3 rounded-lg ${
                    compareType === 'countries' ? 'bg-purple-500/20 text-purple-400' : 'bg-white/5 text-white/40'
                  }`}
                >
                  <CountriesIcon />
                </div>
                ...
                {compareType === 'countries' && <div className="w-2 h-2 rounded-full bg-purple-500" />}
              </div>
              <div className="mt-4 pt-4 border-t border-white/5">
                <div className="flex flex-wrap gap-2">
          ...
          <div className="relative bg-black border border-white/10 p-4 sm:p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {(compareType === 'nodes' ? selectedPubkeys.length : selectedCountryCodes.length) > 0 && (
                <button
                  onClick={compareType === 'nodes' ? () => setSelectedPubkeys([]) : () => setSelectedCountryCodes([])}
