...
export const LeaderboardTotalCreditsCard: React.FC<LeaderboardTotalCreditsCardProps> = ({ 
  ...
  const stats = React.useMemo(() => {
    ...
    return { totalCredits, averageCredits, topCredits, totalPods: credits.length };
  }, [creditsData]);
    ...
    <div className={`relative bg-black border border-white/10 p-6 h-full group hover:border-white/20 transition-all duration-300 overflow-hidden ${className}`}>
      ...
      <div className="flex flex-col justify-center items-center h-full text-center relative z-10">
        ...
        <div className="text-white/40 text-[10px] mb-3">
          across {stats?.totalPods || 0} pods
        </div>
        
        {/* Credits Distribution Bar */}
          ...
          <svg className="w-full" height="24" viewBox="0 0 200 24" preserveAspectRatio="none">
            {Array.from({ length: 45 }).map((_, index) => (
              <rect
                ...
                height={24}
                rx={1}
                fill={index < 35 ? '#eab308' : '#374151'}
              />
            ))}
          </svg>
          <div className="flex justify-center items-center mt-1.5">
            <span className="text-yellow-400 text-[9px] font-medium">avg: {stats ? formatNumber(stats.averageCredits) : '0'} credits</span>
          </div>
        </div>
      </div>
    </div>
